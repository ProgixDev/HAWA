      ...
      <SafeAreaView edges={['top', 'bottom']} style={styles.flex}>
        ...
        <View style={styles.content}>
          <Pressable accessibilityLabel="Retour" onPress={navigation.goBack} style={styles.back}>
            <MaterialDesignIcons color="#35117E" name="arrow-left" size={27} />
          </Pressable>
          ...
          <View style={[styles.keypad, compact && styles.keypadCompact]}>
            {KEYS.map((key, index) => key === 'empty' ? (
              ...
              <Pressable
                ...
                {key === 'delete' ? <MaterialDesignIcons color="#35117E" name="backspace-outline" size={25} /> : <Text style={styles.number}>{key}</Text>}
              </Pressable>
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  title: {color: '#35117E', fontFamily: 'serif', fontSize: 24, fontWeight: '800', textAlign: 'center'},
  titleCompact: {fontSize: 21},
  subtitle: {marginTop: 6, color: '#6A5C78', fontSize: 12},
  gapSmall: {flexGrow: 0.4, minHeight: 18},
  dotsBlock: {alignItems: 'center'},
  dots: {flexDirection: 'row', gap: 15},
  dot: {width: 14, height: 14, borderWidth: 1.5, borderColor: '#A98AC6', borderRadius: 7, backgroundColor: '#FFFFFF'},
  dotFilled: {borderColor: PURPLE, backgroundColor: PURPLE},
  error: {height: 31, marginTop: 10, color: '#8A5370', fontSize: 11, textAlign: 'center'},
  ...
  keyActive: {backgroundColor: '#F1E7F7'},
  number: {color: '#35117E', fontSize: 23, fontWeight: '600'},
