      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.flex}>
        <View style={[styles.header, {paddingTop: veryCompact ? TOP_SPACING_EXTRA_COMPACT : TOP_SPACING_EXTRA}]}>
          <Pressable accessibilityLabel="Retour" accessibilityRole="button" onPress={navigation.goBack} style={styles.headerButton}>
            <MaterialDesignIcons color={DARK} name="chevron-left" size={27} />
          </Pressable>
          ...
          <Pressable accessibilityLabel="Masquer" accessibilityRole="button" onPress={mask} style={styles.mask}>
            <MaterialDesignIcons color={PURPLE} name="eye-off-outline" size={17} />
            ...
            <View style={styles.photoCard}>
              <View style={styles.photoCardHeader}>
                <View style={styles.photoCardHeaderIcon}>
                  <MaterialDesignIcons color={COLORS.purple} name="shield-lock-outline" size={19} />
                </View>
                  ...
                  <View style={[styles.empty, compact && styles.emptyCompact]}>
                    <View style={styles.emptyIconOuter}>
                      <View style={styles.emptyIcon}>
                        <MaterialDesignIcons color={COLORS.purple} name="camera-lock-outline" size={34} />
                      </View>
                      <View style={styles.emptyLockBadge}>
                        <MaterialDesignIcons color="#FFFFFF" name="lock" size={11} />
                      </View>
                    </View>
                ...
                <View style={styles.grid}>
                  ...
                  {canAddMore ? (
                    <Pressable accessibilityLabel="Ajouter une photo" accessibilityRole="button" disabled={busy} onPress={openAddSheet} style={({pressed}) => [styles.gridCell, styles.addCell, pressed && styles.sourceCardPressed, busy && styles.disabled]}>
                      <MaterialDesignIcons color={COLORS.purple} name="plus" size={26} />
              ...
              {pickerError ? (
                <View accessibilityRole="alert" style={styles.errorCard}>
                  <View style={styles.errorIcon}>
                    <MaterialDesignIcons color={COLORS.danger} name="alert-circle-outline" size={18} />
                  </View>
          ...
          <View style={[styles.detailSheet, {paddingBottom: Math.max(insets.bottom, 16)}]}>
            ...
            {selectedPhoto && !selectedPhotoFailed ? (
              ...
              <View style={[styles.detailUnavailable, compact && styles.detailUnavailableCompact]}>
                <MaterialDesignIcons color={COLORS.purple} name="image-off-outline" size={34} />
    ...
    <Pressable accessibilityLabel={failed ? 'Photo indisponible' : 'Voir la photo'} accessibilityRole="button" onPress={onPress} style={styles.gridCell}>
      {failed ? (
        <View style={styles.gridCellUnavailable}>
          <MaterialDesignIcons color={COLORS.muted} name="image-off-outline" size={24} />
        </View>
      ) : display.uri ? (
  ...
  return (
    <Pressable accessibilityLabel={label} accessibilityRole="button" disabled={disabled} onPress={onPress} style={({pressed}) => [styles.sourceCard, tone === 'purple' && styles.galleryCard, pressed && styles.sourceCardPressed, disabled && styles.disabled]}>
      <View style={tone === 'purple' ? styles.sourceCardIconPurple : styles.sourceCardIconNeutral}>
        <MaterialDesignIcons color={tone === 'purple' ? COLORS.purple : COLORS.purpleDark} name={icon} size={27} />
      </View>
      ...
      <View style={styles.sourceArrow}>
        <MaterialDesignIcons color={COLORS.purple} name="chevron-right" size={20} />
      </View>
    </Pressable>
  );
}
  ...
  return (
    <Pressable accessibilityLabel={label} accessibilityRole="button" disabled={disabled} onPress={onPress} style={({pressed}) => [styles.actionCard, danger && styles.actionCardDanger, pressed && styles.sourceCardPressed, disabled && styles.disabled]}>
      <View style={[styles.actionCardIcon, danger && styles.actionCardIconDanger]}>
        <MaterialDesignIcons color={danger ? COLORS.danger : COLORS.purple} name={icon} size={20} />
      </View>
...
const styles = StyleSheet.create({
  ...
  headerButton: {width: 43, height: 43, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: COLORS.border, borderRadius: 15, backgroundColor: COLORS.white},
  headerCopy: {flex: 1, alignItems: 'center', minWidth: 0},
  title: {color: DARK, fontSize: 20, fontWeight: '800'},
  date: {marginTop: 3, color: '#675C79', fontSize: 11},
  mask: {height: 39, flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 9, borderWidth: 1, borderColor: '#E7DEED', borderRadius: 14, backgroundColor: COLORS.white},
  maskText: {color: PURPLE, fontSize: 10, fontWeight: '700'},

  content: {paddingHorizontal: 14, paddingTop: 6, gap: 14},
  subtitle: {color: '#5C5270', fontSize: 13, lineHeight: 19},

  photoCard: {overflow: 'hidden', borderWidth: 1, borderColor: COLORS.border, borderRadius: 24, backgroundColor: COLORS.white, padding: 14, shadowColor: '#3E1F75', shadowOffset: {width: 0, height: 5}, shadowOpacity: 0.07, shadowRadius: 14, elevation: 3},
  photoCardHeader: {flexDirection: 'row', alignItems: 'center', marginBottom: 14},
  photoCardHeaderIcon: {width: 40, height: 40, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 13, backgroundColor: COLORS.lavender},
  photoCardHeaderCopy: {flex: 1, minWidth: 0, marginLeft: 9},
  photoCardEyebrow: {color: COLORS.purple, fontSize: 7.5, fontWeight: '900', letterSpacing: 1},
  photoCardTitle: {marginTop: 2, color: COLORS.purpleDark, fontSize: 14, fontWeight: '800'},
  counterPill: {flexShrink: 0, borderRadius: 11, backgroundColor: COLORS.lavenderStrong, paddingHorizontal: 9, paddingVertical: 6},
  counterPillText: {color: COLORS.purpleDark, fontSize: 10, fontWeight: '800'},
  ...
  emptyIcon: {width: 72, height: 72, alignItems: 'center', justifyContent: 'center', borderRadius: 24, backgroundColor: COLORS.lavender, transform: [{rotate: '-3deg'}]},
  emptyLockBadge: {position: 'absolute', right: 0, bottom: 2, width: 25, height: 25, alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: COLORS.white, borderRadius: 13, backgroundColor: COLORS.purple},
  emptyTitle: {marginTop: 13, color: DARK, fontSize: 17, fontWeight: '800', textAlign: 'center'},
  emptyText: {maxWidth: 300, marginTop: 6, color: COLORS.secondary, fontSize: 12, lineHeight: 18, textAlign: 'center'},

  choiceDivider: {flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10},
  choiceDividerLine: {flex: 1, height: StyleSheet.hairlineWidth, backgroundColor: '#EAE3EE'},
  choiceDividerText: {color: COLORS.muted, fontSize: 7.5, fontWeight: '800', letterSpacing: 0.8},

  sourceCard: {minHeight: 72, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: COLORS.border, borderRadius: 18, backgroundColor: '#FCFAFD', paddingHorizontal: 11, paddingVertical: 10, marginBottom: 9},
  galleryCard: {borderColor: 'rgba(105,66,189,0.20)', backgroundColor: '#F8F3FC'},
  sourceCardPressed: {opacity: 0.8, transform: [{scale: 0.992}]},
  sourceCardIconPurple: {width: 47, height: 47, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 16, backgroundColor: '#E9DDF7'},
  sourceCardIconNeutral: {width: 47, height: 47, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 16, backgroundColor: '#F0EBF4'},
  sourceCardCopy: {flex: 1, minWidth: 0, marginLeft: 11},
  sourceCardTitle: {color: COLORS.purpleDark, fontSize: 13, fontWeight: '800'},
  sourceCardText: {marginTop: 3, color: COLORS.secondary, fontSize: 9.5, lineHeight: 13},
  sourceArrow: {width: 31, height: 31, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 11, backgroundColor: COLORS.white},

  grid: {flexDirection: 'row', flexWrap: 'wrap', gap: 10},
  gridCell: {width: '47%', aspectRatio: 1, borderRadius: 18, overflow: 'hidden', backgroundColor: COLORS.lavenderSoft},
  gridCellImage: {width: '100%', height: '100%'},
  gridCellUnavailable: {flex: 1, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderStyle: 'dashed', borderColor: COLORS.lavenderStrong, backgroundColor: COLORS.lavenderSoft},
  gridCellLoading: {flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.lavenderSoft},
  addCell: {borderWidth: 1, borderStyle: 'dashed', borderColor: 'rgba(105,66,189,0.35)', backgroundColor: '#F8F3FC', alignItems: 'center', justifyContent: 'center', gap: 6},
  addCellText: {color: COLORS.purple, fontSize: 11.5, fontWeight: '800'},

  actionSectionLabel: {marginTop: 14, marginBottom: 8, color: COLORS.text, fontSize: 11.5, fontWeight: '800'},
  actionCardsRow: {flexDirection: 'row', gap: 9},
  actionCard: {flex: 1, minWidth: 0, minHeight: 74, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#DDD0EA', borderRadius: 17, backgroundColor: '#F8F4FC', paddingHorizontal: 9, paddingVertical: 9},
  actionCardDanger: {borderColor: '#EAC3C8', backgroundColor: COLORS.dangerSoft},
  actionCardIcon: {width: 36, height: 36, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: '#EAE0F6'},
  actionCardIconDanger: {backgroundColor: '#F4DFE2'},
  actionCardCopy: {flex: 1, minWidth: 0, marginLeft: 8},
  actionCardTitle: {color: COLORS.purpleDark, fontSize: 11, fontWeight: '800'},
  actionCardTitleDanger: {color: COLORS.danger},
  actionCardSubtitle: {marginTop: 2, color: COLORS.secondary, fontSize: 8, lineHeight: 11},

  errorCard: {flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 11, borderRadius: 14, backgroundColor: COLORS.dangerSoft, paddingHorizontal: 10, paddingVertical: 9},
  errorIcon: {width: 30, height: 30, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 10, backgroundColor: '#F2DDE0'},
  pickerErrorText: {flex: 1, color: COLORS.danger, fontSize: 10.5, lineHeight: 15},

  security: {minHeight: 58, flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 12, borderWidth: 1, borderColor: 'rgba(105,66,189,0.08)', borderRadius: 17, backgroundColor: COLORS.lavender},
  securityIcon: {width: 36, height: 36, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: COLORS.white},
  securityText: {flex: 1, color: '#55436C', fontSize: 10.5, lineHeight: 15},

  save: {height: 54, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 18, backgroundColor: PURPLE, shadowColor: '#5731A3', shadowOffset: {width: 0, height: 5}, shadowOpacity: 0.2, shadowRadius: 10, elevation: 4},
  saveText: {color: '#FFFFFF', fontSize: 16, fontWeight: '800'},
  pressed: {opacity: 0.82, transform: [{scale: 0.985}]},
  disabled: {opacity: 0.55},

  overlay: {flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(31,18,61,0.42)'},
  handle: {width: 44, height: 5, alignSelf: 'center', borderRadius: 3, backgroundColor: '#D8CBE3', marginBottom: 14},

  detailSheet: {borderTopLeftRadius: 26, borderTopRightRadius: 26, backgroundColor: COLORS.white, paddingHorizontal: 16, paddingTop: 12},
  detailImage: {width: '100%', height: 320, borderRadius: 18, backgroundColor: COLORS.lavenderSoft},
  detailImageCompact: {height: 220},
  detailLoading: {alignItems: 'center', justifyContent: 'center'},
  detailUnavailable: {width: '100%', height: 220, borderRadius: 18, borderWidth: 1, borderStyle: 'dashed', borderColor: COLORS.lavenderStrong, backgroundColor: COLORS.lavenderSoft, alignItems: 'center', justifyContent: 'center', gap: 10, paddingHorizontal: 24},
  detailUnavailableCompact: {height: 160},
  detailUnavailableText: {color: COLORS.secondary, fontSize: 12.5, lineHeight: 18, textAlign: 'center'},
  detailActionsRow: {flexDirection: 'row', gap: 9, marginTop: 14},

  sourceSheet: {borderTopLeftRadius: 26, borderTopRightRadius: 26, backgroundColor: COLORS.white, paddingHorizontal: 16, paddingTop: 12},
  sourceSheetTitle: {marginBottom: 14, color: DARK, fontSize: 17, fontWeight: '800'},
});
