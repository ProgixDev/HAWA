...
function PinSetupScreen({navigation}: Props): React.JSX.Element {
  ...
  return <ImageBackground source={BACKGROUND} resizeMode="cover" style={styles.page}><SafeAreaView style={styles.safeArea}><StatusBar translucent backgroundColor="transparent" barStyle="dark-content" /><View style={[styles.content, compact && styles.contentCompact, {paddingBottom: Math.max(insets.bottom, 16)}]}>
    ...
    <View style={[styles.keypad, compact && styles.keypadCompact]}>{keys.map((key, index) => key === 'empty' ? <View key={key} style={[styles.key, compact && styles.keyCompact]} /> : <Pressable accessibilityLabel={key === 'delete' ? 'Effacer' : key} key={`${key}-${index}`} onPress={() => enter(key)} style={({pressed}) => [styles.key, compact && styles.keyCompact, styles.keyActive, pressed && styles.pressed]}>{key === 'delete' ? <Image accessibilityIgnoresInvertColors source={DELETE_ICON} style={styles.deleteIcon} /> : <><Text style={styles.number}>{key}</Text>{letters[key] && <Text style={styles.letters}>{letters[key]}</Text>}</>}</Pressable>)}</View>
...
const styles = StyleSheet.create({page:{flex:1,backgroundColor:'#FBF6EC'},safeArea:{flex:1},content:{flex:1,alignItems:'center',paddingTop:64,paddingBottom:32,paddingHorizontal:spacing.lg},contentCompact:{paddingTop:16},iconMedallion:{width:58,height:58,alignItems:'center',justifyContent:'center',borderRadius:29,backgroundColor:'#F5EBD9'},lockIcon:{width:68,height:68,resizeMode:'contain'},title:{marginTop:20,color:'#14201D',fontFamily:'serif',fontSize:22,fontWeight:'600'},subtitle:{marginTop:9,color:'#55605C',fontSize:13,lineHeight:19,textAlign:'center'},dots:{flexDirection:'row',gap:22,marginTop:32,marginBottom:31},dot:{width:15,height:15,borderWidth:1.5,borderColor:'#B7A88F',borderRadius:8,backgroundColor:'#FFFDF8'},dotFilled:{borderColor:'#176548',backgroundColor:'#176548'},keypad:{width:236,flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between',rowGap:14},keypadCompact:{width:206,rowGap:7},key:{width:62,height:62,alignItems:'center',justifyContent:'center',borderRadius:31},keyCompact:{width:54,height:54,borderRadius:27},keyActive:{backgroundColor:'#F6EDDE'},deleteIcon:{width:46,height:46,resizeMode:'contain'},number:{color:'#15201D',fontSize:23,lineHeight:26},letters:{color:'#40504A',fontSize:7,fontWeight:'700',letterSpacing:1},spacer:{flex:1},later:{color:'#176548',fontSize:13,fontWeight:'600'},pressed:{opacity:0.65}});
