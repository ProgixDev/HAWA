...
function PinSetupScreen({navigation}: Props): React.JSX.Element {
  ...
  const enter = (key: typeof keys[number]) => {
    ...
    if (next.length === 4) {setTimeout(() => Alert.alert('Code PIN créé', 'Ton espace est maintenant protégé.'), 180);}
  };
  return <View style={styles.page}><SafeAreaView style={styles.safeArea}><StatusBar translucent backgroundColor="transparent" barStyle="dark-content" /><View style={styles.content}>
    ...
    <View style={styles.keypad}>{keys.map((key, index) => key === 'empty' ? <View key={key} style={styles.key} /> : <Pressable accessibilityLabel={key === 'delete' ? 'Effacer' : key} key={`${key}-${index}`} onPress={() => enter(key)} style={({pressed}) => [styles.key, styles.keyActive, pressed && styles.pressed]}>{key === 'delete' ? <MaterialDesignIcons color="#173F35" name="backspace-outline" size={23} /> : <><Text style={styles.number}>{key}</Text>{letters[key] && <Text style={styles.letters}>{letters[key]}</Text>}</>}</Pressable>)}</View>
    <View style={styles.spacer} /><Pressable onPress={() => navigation.goBack()}><Text style={styles.later}>Plus tard</Text></Pressable>
  </View></SafeAreaView></View>;
}

const styles = StyleSheet.create({page:{flex:1,backgroundColor:'#FBF6EC'},safeArea:{flex:1},content:{flex:1,alignItems:'center',paddingTop:64,paddingBottom:32,paddingHorizontal:spacing.lg},iconMedallion:{width:58,height:58,alignItems:'center',justifyContent:'center',borderRadius:29,backgroundColor:'#F5EBD9'},lockIcon:{width:68,height:68,resizeMode:'contain'},title:{marginTop:20,color:'#14201D',fontFamily:'serif',fontSize:22,fontWeight:'600'},subtitle:{marginTop:9,color:'#55605C',fontSize:13,lineHeight:19,textAlign:'center'},dots:{flexDirection:'row',gap:22,marginTop:32,marginBottom:31},dot:{width:15,height:15,borderWidth:1.5,borderColor:'#B7A88F',borderRadius:8,backgroundColor:'#FFFDF8'},dotFilled:{borderColor:'#176548',backgroundColor:'#176548'},keypad:{width:236,flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between',rowGap:14},key:{width:62,height:62,alignItems:'center',justifyContent:'center',borderRadius:31},keyActive:{backgroundColor:'#F6EDDE'},number:{color:'#15201D',fontSize:23,lineHeight:26},letters:{color:'#40504A',fontSize:7,fontWeight:'700',letterSpacing:1},spacer:{flex:1},later:{color:'#176548',fontSize:13,fontWeight:'600'},pressed:{opacity:0.65}});
