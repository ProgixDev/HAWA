...
const CYCLE_RETURN_LABELS: Record<
...
const TRYING_AGAIN_LABELS: Record<
...
function countByOption<
  T extends MiscarriageJournalEntry,
>(
  entries: T[],
  field: keyof T,
  options: string[],
  ...
  entries.forEach(entry => {
    const value =
      entry[field] as unknown as string;
...
function countBySymptom(
  entries:
    MiscarriageJournalEntry[],
): Array<{
  label: string;
  count: number;
}> {
  ...
  entries.forEach(entry => {
    entry.physicalSymptoms?.forEach(
      symptom => {
        counts.set(
          symptom,
          (counts.get(symptom) ??
  ...
  return MISCARRIAGE_PHYSICAL_SYMPTOMS
    .map(label => ({
      label,
      count:
        counts.get(label) ?? 0,
...
function MiscarriageStatisticsScreen(): React.JSX.Element {
  ...
  const miscarriageDate =
    useMemo(
      () =>
        prefs.miscarriageDate
          ? startOfDay(
              new Date(
                `${prefs.miscarriageDate}T12:00:00`,
              ),
            )
          : null,
      [
  ...
  const firstReturnedPeriodDate =
    useMemo(
      () =>
        prefs.firstReturnedPeriodDate
          ? new Date(
              `${prefs.firstReturnedPeriodDate}T12:00:00`,
            )
          : null,
      [
        prefs.firstReturnedPeriodDate,
  ...
  const mostFrequentBleeding =
    bleedingCounts[0]
      ?.label;

  const maxBleedingCount =
    Math.max(
      ...bleedingCounts.map(
        item => item.count,
      ),
      ...
      () =>
        bleedingEntries
          ...
          .map(entry => ({
            ...
            value:
              MISCARRIAGE_BLEEDING_OPTIONS.indexOf(
                entry.bleeding as string,
              ) + 1,
          })),
      [
        bleedingEntries,
  ...
  const tryingAgainCounts =
    useMemo(
      () =>
        countByOption(
          tryingAgainEntries,
          'tryingAgain',
          MISCARRIAGE_TRYING_AGAIN_OPTIONS.map(
            option =>
              option.id,
          ),
        ).map(item => ({
          label:
            TRYING_AGAIN_LABELS[
              item.label as MiscarriageTryingAgainStatus
            ] ??
            item.label,
  ...
  const maxTryingAgainCount =
    Math.max(
      ...tryingAgainCounts.map(
        item => item.count,
      ),
      1,
    );
    ...
    <>
      <AnimatedSection>
        <View
          ...
          <View
            style={
              styles.trackingValueRow
            }>
            <Text
              style={
                styles.trackingBigValue
              }>
              {
                notesCount
              }
