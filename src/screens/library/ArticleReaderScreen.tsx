...
import {getArticleById, getCategoryById, LIBRARY_TINTS, RELIGIOUS_DISCLAIMER} from '../../data/libraryContent';
...
const LEVEL_LABEL = {beginner: 'Débutant', intermediate: 'Intermédiaire', advanced: 'Avancé'} as const;
...
const BESPOKE_ARTICLE_SCREENS: Partial<Record<string, React.ComponentType<Props>>> = {
...
function ArticleReaderScreen(props: Props): React.JSX.Element {
  ...
  if (BespokeScreen) {return <BespokeScreen {...props} />;}
  return <GenericArticleReaderScreen {...props} />;
}

function GenericArticleReaderScreen({route, navigation}: Props): React.JSX.Element {
  const {articleId} = route.params;
  ...
  if (!article) {
    return (
      <View style={[styles.missingContainer, {paddingTop: getTopPadding(insets.top, true), paddingBottom: getBottomPadding(insets.bottom)}]}>
        <Text style={styles.missingText}>Cet article n’est plus disponible.</Text>
  ...
  return (
    <View style={styles.safeArea}>
      ...
      <ScrollView
        onScroll={event => saveScrollPosition(articleId, event.nativeEvent.contentOffset.y)}
        ref={scrollRef}
        scrollEventThrottle={200}
        showsVerticalScrollIndicator={false}
        style={styles.scroll}>
        <View style={[styles.hero, {backgroundColor: tint.bg, paddingTop: getTopPadding(insets.top, true)}]}>
          <Pressable
            ...
            <MaterialDesignIcons color={tint.fg} name="chevron-left" size={24} />
          </Pressable>

          <BookmarkButton active={bookmarked} onPress={handleToggleBookmark} tone="light" />

          <View style={styles.heroIconWrap}>
            <MaterialDesignIcons color={tint.fg} name={category?.icon ?? 'book-open-page-variant-outline'} size={44} />
          </View>
        </View>
        ...
        <View style={styles.body}>
          <View style={styles.pillRow}>
            {category ? (
              <View style={[styles.categoryPill, {backgroundColor: tint.bg}]}>
                <Text style={[styles.categoryPillText, {color: tint.fg}]}>{category.label}</Text>
              </View>
            ) : null}
            {isReligious && (
              <View style={styles.educationalPill}>
                <MaterialDesignIcons color="#B7791F" name="school-outline" size={11} />
          ...
          {isReligious && (
            <View style={styles.disclaimer}>
              <MaterialDesignIcons color="#5C4212" name="information-outline" size={16} />
          ...
          <View style={{height: getBottomPadding(insets.bottom, READING_CONTROLS_SPACE)}} />
        </View>
      </ScrollView>

      <ReadingControls articleId={articleId} durationMinutes={article.durationMinutes} scrollRef={scrollRef} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  categoryPill: {alignSelf: 'flex-start', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4},
  ...
  title: {marginTop: 12, color: homeColors.textPrimary, fontFamily: 'serif', fontSize: 23, fontWeight: '700', lineHeight: 29},
  metaRow: {flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 10},
  metaText: {color: homeColors.textSecondary, fontSize: 12},
  metaDot: {color: homeColors.textSecondary, fontSize: 12},
  summary: {marginTop: 14, color: homeColors.textPrimary, fontSize: 15, fontWeight: '600', lineHeight: 21},
  divider: {height: 1, backgroundColor: homeColors.cardBorder, marginTop: 18, marginBottom: 4},
  paragraph: {marginTop: 16, color: homeColors.textPrimary, fontSize: 14.5, lineHeight: 23},
  pressed: {opacity: 0.85},
  missingContainer: {flex: 1, alignItems: 'center', justifyContent: 'center', gap: 12, backgroundColor: '#FCFAFF'},
  missingText: {color: homeColors.textSecondary, fontSize: 14},
  missingButton: {minHeight: 44, paddingHorizontal: 20, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: homeColors.primary},
