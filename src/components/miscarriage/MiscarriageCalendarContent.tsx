...
function MiscarriageCalendarContent(): React.JSX.Element {
  ...
  const [visibleFilters, setVisibleFilters] = useState<
    Set<MiscarriageJournalCategory>
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      hydrateMiscarriageJournal().then(() => {
        if (active) {
          setEntries(getAllMiscarriageJournalEntries());
        }
      });
  ...
  const daysSinceEvent = miscarriageDate
    ? Math.max(0, diffDays(selectedDate, miscarriageDate))
    : null;
  const isMiscarriageDaySelected =
    Boolean(miscarriageDate) && sameDay(selectedDate, miscarriageDate as Date);
  const isReturnedPeriodDaySelected =
    Boolean(firstReturnedPeriodDate) &&
    sameDay(selectedDate, firstReturnedPeriodDate as Date);
          ...
          <View style={styles.calendarCard}>
            ...
            <View style={styles.daysGrid}>
              {days.map((date, index) => {
                ...
                const isMiscarriageDay =
                  Boolean(miscarriageDate) &&
                  sameDay(date, miscarriageDate as Date);
                const isReturnedPeriodDay =
                  Boolean(firstReturnedPeriodDate) &&
                  sameDay(date, firstReturnedPeriodDate as Date);
                ...
                return (
                  <View key={date.toISOString()} style={styles.dayCell}>
                    <Pressable
                      ...
                      }${isReturnedPeriodDay ? ', retour des règles' : ''}`}
              ...
              <View style={styles.dayBadgeRow}>
                {isMiscarriageDaySelected ? (
                  <View style={styles.eventBadge}>
                    <MaterialDesignIcons
                      color={CATEGORY_META.bleeding.color}
                      name="flower-outline"
                      size={14}
                    />
