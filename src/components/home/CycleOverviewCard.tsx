...
function OverviewColumn({item, compact}: {item: OverviewItem; compact: boolean}) {
  return (
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.row}>
        {items.map(item => (
          <OverviewColumn compact={compact} item={item} key={item.key} />
        ))}
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  more: {color: homeColors.primary, fontSize: 12.5, fontWeight: '700'},
