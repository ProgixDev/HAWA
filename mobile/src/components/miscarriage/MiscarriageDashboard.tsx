...
// PHASE D7 — PURPLE/DEEP_PURPLE/SOFT_PURPLE_2/BORDER/MUTED/SUCCESS used to be
// fixed literals here; they are now derived from useAwaTheme() inside
// MiscarriageDashboard() (and re-derived identically inside createStyles(theme)
// and createTransitionModalStyles(theme), since ConceiveTransitionModal is a
// sibling top-level component that can no longer close over module-scope
// values). No medical/recovery-semantic color exists in this file — see the
// final report for the full color-classification audit. The "Retour du
// cycle" stat card's pink icon is a phase-invariant decorative accent (never
// varies by cycleReturnStatus), same finding as Postpartum D6's lochia icon
// and Conceive D4's hero badge.
...
// PHASE D7 — converted to a createTransitionModalStyles(theme) factory,
// same pattern as every dashboard's main createStyles(theme). This modal's
// backdrop stays fixed (a scrim, not a surface — matches every prior
// dashboard's modal-dim precedent).
function createTransitionModalStyles(theme: ResolvedAwaTheme) {
...
function MiscarriageDashboard({ navigation }: Props): React.JSX.Element {
  ...
  // DEEP_PURPLE/MUTED/SUCCESS/BORDER/SOFT_PURPLE_2 are only used inside
  // createStyles(theme) below, not in this component's own JSX, so they're
  // re-derived there instead. No medical/recovery-semantic color exists in
  // this file to protect — see the final report's color-classification audit.
  ...
  const quickActionItems: QuickActionItem[] = [
    ...
    // the canonical '#B23F63'/'#F9DCE8' used by every other dashboard's
    // daily-journal action — a pre-existing cross-dashboard inconsistency,
    // documented and left exactly as-is rather than silently "fixed" to
    // match (out of scope for a theme migration).
    {
      key: 'daily-journal',
    ...
    // Category E (fixed action-identity accent). Slightly different from
    // the canonical '#2C8E93'/'#DDF0F1' used elsewhere — same pre-existing
    // inconsistency note as daily-journal above.
    {
      key: 'statistics',
      icon: 'chart-donut',
      iconColor: '#328C92',
              ...
              <View style={styles.heroCard}>
                ...
                <View style={styles.heroContentRow}>
                  <View style={styles.heroStatsRow}>
                    ...
                    {/* PHASE D7 — this stat card's pink icon
                        (theme.colors.secondary, was fixed '#B65376') is a
                        phase-invariant decorative accent, NOT a protected
                        medical-severity color: it never changes based on
                        the actual cycleReturnStatus (no/yes/unknown),
                  ...
                  return (
                    ...
                    >
                      <View style={styles.dailyItemTop}>
                        ...
                        >
                          <MaterialDesignIcons
                            color={done ? onPrimaryTextColor(theme) : PURPLE}
                            name={item.icon}
                            size={22}
                          />
                        </View>
...
// PHASE D7 — converted to a createStyles(theme) factory, same pattern as
// every prior dashboard (D1-D6). PURPLE/DEEP_PURPLE/SOFT_PURPLE_2/BORDER/
// MUTED/SUCCESS are re-derived here (same mapping as the component-body
// copy above) so every style below keeps working unchanged by name. The 2
// Category E quick-action accents (daily-journal rose, statistics teal)
// stay fixed — see their own comment at the JSX call site.
function createStyles(theme: ResolvedAwaTheme) {
  const PURPLE = theme.colors.primary;
  const DEEP_PURPLE = theme.colors.accent;
  const SOFT_PURPLE_2 = theme.colors.primarySoft;
  ...
  heroStatCard: {
    ...
    borderRadius: 18,
    backgroundColor: withAlpha(theme.colors.surface, 0.93),
  },
  ...
  // site's own comment for why this stays phase-invariant decorative, not
  // a protected medical-severity color.
  heroStatIconPink: {
    width: 38,
    height: 38,
    alignItems: 'center',
  ...
  supportIcon: {
    ...
    justifyContent: 'center',
    borderRadius: 10,
    backgroundColor: withAlpha(theme.colors.surface, 0.74),
  },
  ...
  // Generic "logged today" completion text — success token, not a
  // symptom-severity claim (same DailyJournalCard/Irregular D3/Postpartum
  // D6 precedent), unified with dailyStateDotDone/doneBadge's own success
  // fill for internal consistency (the original used a slightly different,
  // muted-green literal here).
  dailyStateTextDone: {
    color: SUCCESS,
  },
