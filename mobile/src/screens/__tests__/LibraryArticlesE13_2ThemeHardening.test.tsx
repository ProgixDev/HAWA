...
import FirstPeriodDailyLifeArticleScreen from '../library/FirstPeriodDailyLifeArticleScreen';
...
import OvulationArticleScreen from '../library/OvulationArticleScreen';
import CervicalMucusArticleScreen from '../library/CervicalMucusArticleScreen';
import BasalTemperatureArticleScreen from '../library/BasalTemperatureArticleScreen';
import LhTestsArticleScreen from '../library/LhTestsArticleScreen';
import NutritionCycleArticleScreen from '../library/NutritionCycleArticleScreen';

// E13.2 — Library bespoke article screens, Batch 2 (First Period + Fertility
// Tracking, 13 files). Every file previously hardcoded its own local
// CREAM/INK/ROSE(or PURPLE)/BORDER(+BODY/MUTED/GREEN) palette with zero
// useAwaTheme() and a fixed `barStyle="dark-content"`. Unlike E13.1's
// FlowColorsTexturesArticleScreen (which legitimately kept real flow-color
// educational swatches), a deliberate re-audit of all 4 fertility-tracking
// data screens (CervicalMucus/BasalTemperature/LhTests/NutritionCycle) found
// NO genuine data/educational color-coding — every one of the 13 files in
// this batch is 100% migrated with zero remaining hex/rgba literals.
...
/* ============================================================
   STATIC GUARD — all 13 E13.2 target files.
============================================================ */

const TARGET_FILES: Array<[string, string]> = [
  ['FirstPeriodArticleScreen', '../library/FirstPeriodArticleScreen.tsx'],
  ['FirstPeriodSignsArticleScreen', '../library/FirstPeriodSignsArticleScreen.tsx'],
  ['FirstPeriodComingArticleScreen', '../library/FirstPeriodComingArticleScreen.tsx'],
  ['FirstPeriodProtectionArticleScreen', '../library/FirstPeriodProtectionArticleScreen.tsx'],
  ['FirstPeriodDailyLifeArticleScreen', '../library/FirstPeriodDailyLifeArticleScreen.tsx'],
  ['FirstPeriodIrregularArticleScreen', '../library/FirstPeriodIrregularArticleScreen.tsx'],
  ['FirstPeriodFaqArticleScreen', '../library/FirstPeriodFaqArticleScreen.tsx'],
  ['FertilityWindowArticleScreen', '../library/FertilityWindowArticleScreen.tsx'],
  ['OvulationArticleScreen', '../library/OvulationArticleScreen.tsx'],
  ['CervicalMucusArticleScreen', '../library/CervicalMucusArticleScreen.tsx'],
  ['BasalTemperatureArticleScreen', '../library/BasalTemperatureArticleScreen.tsx'],
  ['LhTestsArticleScreen', '../library/LhTestsArticleScreen.tsx'],
  ['NutritionCycleArticleScreen', '../library/NutritionCycleArticleScreen.tsx'],
];

describe('E13.2 Library articles (First Period + Fertility Tracking) — static appearance-resolution guard', () => {
  ...
  it('all 13 files are fully migrated — zero remaining structural hex/rgba literals (no genuine educational/data-semantic colors were found in this batch)', () => {
  ...
  it('NutritionCycleArticleScreen uses onPrimaryTextColor(theme) for text on its primary-filled meal tag, not a hardcoded white', () => {
    const source = fs.readFileSync(path.resolve(__dirname, '../library/NutritionCycleArticleScreen.tsx'), 'utf8');
...
const RUNTIME_SCREENS: Array<[string, React.ComponentType<any>]> = [
  ['FirstPeriodArticleScreen', FirstPeriodArticleScreen],
  ['FirstPeriodFaqArticleScreen', FirstPeriodFaqArticleScreen],
  ['FertilityWindowArticleScreen', FertilityWindowArticleScreen],
  ['CervicalMucusArticleScreen', CervicalMucusArticleScreen],
  ['BasalTemperatureArticleScreen', BasalTemperatureArticleScreen],
  ['LhTestsArticleScreen', LhTestsArticleScreen],
  ['NutritionCycleArticleScreen', NutritionCycleArticleScreen],
];
...
describe('remaining E13.2 screens render without crashing under the theme provider', () => {
  const OTHER_SCREENS: Array<[string, React.ComponentType<any>]> = [
    ['FirstPeriodSignsArticleScreen', FirstPeriodSignsArticleScreen],
    ['FirstPeriodComingArticleScreen', FirstPeriodComingArticleScreen],
    ['FirstPeriodProtectionArticleScreen', FirstPeriodProtectionArticleScreen],
    ['FirstPeriodDailyLifeArticleScreen', FirstPeriodDailyLifeArticleScreen],
    ['FirstPeriodIrregularArticleScreen', FirstPeriodIrregularArticleScreen],
    ['OvulationArticleScreen', OvulationArticleScreen],
  ];
